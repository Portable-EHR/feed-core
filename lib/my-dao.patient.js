...
const { normalizeName, normalizePhoneNumber, dbMsg, } = require('../../nodecore/lib/utils');
const { dbInsert, dbUpdate, dbDelete, fetchFromDb, DbTransaction, } = require('../../nodecore/lib/my-dao');
const { FeedRecord, FeedItemRecord, ReferencedFeedRecordSetup, WrongType, OnlyInserted, InsertAndUpdated,
        RecordJoined, Referenced, UniOwned, MultiOwned, Validation, } = require('./my-dao');
const { PractitionerRecord } = require('./my-dao.practitioner');

const { EIssuerKind, EGender, ELanguage, } = require('./dao.shared');
const {
...
Object.assign(self, { EIssuerKind, EGender, ELanguage, });
...
class MultiAddressRecord extends FeedRecord {
    ...
    constructor({id, row_version, row_created, row_persisted,
        ...
        // Object.assign(this, {backend_item_id, street_1, street_2, city, state, zip, country });
    }

    //region The get/set "interface": {  feedItemId, backendItemId, street1, street2, city, state, zip, country, ... }
...
class ContactRecord extends FeedRecord {
    ...
    constructor({id, row_version, row_created, row_persisted,  feed_item_id, backend_item_id,
                    last_name, first_name, middle_name, normalized_name,
        ...
        this._setMostRecentKnownPersistedRowOwnUpdatableValues({ backend_item_id,
                ...
                preferred_gender, preferred_language, email, alternate_email, mobile_phone, land_phone, fax,
                salutation, pro_salutation, titles, address_id, });
        // Object.assign(this, { last_name, first_name, middle_name, normalized_name, preferred_gender, preferred_language,
        //      email, alternate_email, mobile_phone, land_phone, fax, salutation, pro_salutation, titles, address_id, });
    }

    //region The get/set "interface": {  feed_item_id,feedItemId,  backendItemId, lastName, firstName, middleName, ePreferredGender, preferredLanguage, email, alternateEmail, mobilePhone, landPhone, fax, salutation, proSalutation, titles, ... }
    ...
    toOwnLitOb() {
        const { feedItemId, backendItemId, lastName, firstName, middleName, normalizedName, ePreferredGender, landPhone,
                preferredLanguage, email, alternateEmail, mobilePhone, fax, salutation, proSalutation, titles } = this;
        return {feedItemId, backendItemId, lastName, firstName, middleName, normalizedName, ePreferredGender,
                preferredLanguage, email, alternateEmail, mobilePhone, landPhone, fax, salutation,proSalutation,titles};
    }
     ...
     *        }} srcOb
     * @param {Error[]|undefined} validationErrors
     * @param {function|undefined} _dbInsert
     * @param {function|undefined} _fetchFromDb
     * @param {DbTransaction|null|undefined} trans
     * @returns {Promise<ContactRecord>}
     * @constructor
     */
    static async Insert({srcOb, validationErrors=[], _dbInsert=dbInsert, _fetchFromDb=fetchFromDb, trans=null}) {
        const { lastName, firstName, middleName, mobilePhone, landPhone, fax } = srcOb;
        Object.assign(srcOb, {
            //  todo: ideally, normalizeName() would be a MY-DB PROCEDURE, TRIGGERed on INSERT and UPDATE.
            normalizedName: normalizeName(lastName, firstName, middleName),               //  add normalizedName
        ...
        //  HACK !
        //          Though preferred_language is a string in DB, we want to pre-validate it as a ELanguage.
        this.ValidateAsEnum(ContactRecord._OwnFields.preferred_language, ELanguage, srcOb, validationErrors);
    ...
    async updateWithCandidate(nativeSrcOb, { validationErrors=[], conflicts=[], _dbInsert=dbInsert, _dbUpdate=dbUpdate,
        ...
        const { lastName, firstName, middleName, mobilePhone, landPhone, fax } = nativeSrcOb;

        Object.assign(nativeSrcOb, {
            normalizedName: normalizeName(lastName, firstName, middleName),
        ...
        //  HACK !
        //          Though preferred_language is a string in DB, we want to pre-validate it as a ELanguage.
        this.constructor.ValidateAsEnum(ContactRecord._OwnFields.preferred_language, ELanguage, nativeSrcOb, validationErrors);
...
class FullContactJoined extends ContactJoined {
    address(joinName) { return UniOwned({joinName, FeedRecord:CivicAddressRecord,}); }
}
...
class MultiContactJoined extends FullContactJoined {
    addresses(joinName) { return MultiOwned({joinName, FeedRecord:MultiAddressRecord,}); }
}
class MultiContactRecord extends FullContactRecord {
    ...
    //region The get/set "interface": { address, addresses }
...
class PatientLegitIdRecord extends FeedRecord {
    ...
    constructor({id, row_version, row_created, row_persisted, patient_id, feed_item_id, backend_item_id, issuer_kind, issuer_alias, number, version, expires_on}) {
        ...
        // Object.assign(this, { backend_item_id, issuer_kind, issuer_alias, number, version, expires_on });
    }

    //region The get/set "interface": { patient_id, feedItemId, backendItemId, eIssuerKind, issuerAlias, number, version, expiresOn, ... }
...
class PatientJoined extends RecordJoined {
    primaryFeedPractitionerId(joinName) { return Referenced({joinName, FeedRecord:PractitionerRecord, referencePropertyName:'primaryPractitioner', }); }

    birthPlace(               joinName) { return   UniOwned({joinName, FeedRecord:BirthPlaceRecord,     ownerLitObName:'placeOfBirth',       colNameJoiningToOwned: 'birth_place_id',       }); }
    motherBirthPlace(         joinName) { return   UniOwned({joinName, FeedRecord:BirthPlaceRecord,     ownerLitObName:'motherPlaceOfBirth', colNameJoiningToOwned: 'mother_birth_place_id',}); }
    /**
     *
     * @returns {{params: JoinedParams, Joined: FullContactJoined}}
     */
    selfContact(              joinName) { return   UniOwned({joinName, FeedRecord:FullContactRecord,    ownerLitObName:'contact',            colNameJoiningToOwned: 'self_contact_id',      }); }
    emergencyContact(         joinName) { return   UniOwned({joinName, FeedRecord:ContactRecord,                                             colNameJoiningToOwned: 'emergency_contact_id', }); }

    legitIds(                 joinName) { return MultiOwned({joinName, FeedRecord:PatientLegitIdRecord, ownerLitObArrayName:'identifiedBy',         }); }
}
class PatientRecord extends FeedItemRecord {
    ...
    constructor({id, row_version, row_created, row_persisted, row_retired,  feed_alias, feed_item_id, backend_item_id,
                    ...
                    family_name_at_birth, first_name_at_birth, middle_name_at_birth, normalized_name_at_birth,
    ...
    //region The get/set "interface": {  chartNumber, dateOfBirth, birthPlace, eGenderAtBirth, eGender, familyNameAtBirth ... }
    ...
    // noinspection JSUnusedGlobalSymbols
    get normalizedNameAtBirth() { throw Error(`${this.Name}.prototype.get normalizedNameAtBirth() : Not defined yet. Run ${this.Name}.Setup().`); }
    // noinspection JSUnusedGlobalSymbols
    set normalizedNameAtBirth(value) { throw Error(`${this.Name}.prototype.set normalizedNameAtBirth(value=${value}) : Not defined yet. Run ${this.Name}.Setup().`); }
    ...
    toOwnLitOb() {
        const {  chartNumber, dateOfBirth, eGenderAtBirth, eGender, familyNameAtBirth, firstNameAtBirth,
            middleNameAtBirth, normalizedNameAtBirth, familyName, firstName, middleName, normalizedName,
        ...
        return { chartNumber, dateOfBirth, eGenderAtBirth, eGender, familyNameAtBirth, firstNameAtBirth,
            middleNameAtBirth, normalizedNameAtBirth, familyName, firstName, middleName, normalizedName,
            motherFamilyNameAtBirth, motherFirstNameAtBirth, motherMiddleNameAtBirth, motherDateOfBirth, dateOfDeath, };
    }
    ...
    static _OwnFields = {
        ...
        normalized_name_at_birth(   colName) {  return InsertAndUpdated({colName, recName:'normalizedNameAtBirth',  }); },
    ...
    static get DbManagedColumnNames() {
        return super.DbManagedColumnNames.concat([ this._OwnFields.chart_number.name, ]);
    }
    ...
    static async Insert({srcOb, validationErrors=[]}) {
        ...
        let { familyNameAtBirth=null, firstNameAtBirth=null, middleNameAtBirth=null, } = srcOb;
        ...
        Object.assign(srcOb, {  //  set default non-null '' for chartNumber, INSERT TRIGGER will handle the rest.
            ...
            familyNameAtBirth, firstNameAtBirth, middleNameAtBirth,
            normalizedNameAtBirth : normalizeName(familyNameAtBirth, firstNameAtBirth, middleNameAtBirth),
     ...
     * @param {function} _fetchFromDb
     * @param {DbTransaction|null} trans
     * @return {Promise<PatientRecord>}
     */
    async updateWithCandidate(nativeSrcOb, {validationErrors=[], conflicts=[], _dbInsert=dbInsert, _dbUpdate=dbUpdate,
                                                        _dbDelete=dbDelete, _fetchFromDb=fetchFromDb, trans=null}={}) {
        const { familyName, firstName, middleName,
                familyNameAtBirth, firstNameAtBirth, middleNameAtBirth, } = nativeSrcOb;

        Object.assign(nativeSrcOb, {
            normalizedNameAtBirth : normalizeName(familyNameAtBirth, firstNameAtBirth, middleNameAtBirth),
            normalizedName: normalizeName(familyName, firstName, middleName),
        });
        return await super.updateWithCandidate(nativeSrcOb, { validationErrors, conflicts, _dbInsert, _dbUpdate,
                                                                                _dbDelete, _fetchFromDb, trans});
    }
    ...
    static async DaoSearch(searchStr, feed) {
        ...
        const searchStrs = This._SearchStrs(searchStr);     //  'allo Gi, 34 d35'  =>  ['allo', 'Gi', '34', 'd35']

        const phoneCriteria = This._PhoneSearchCirteria(searchStrs);
        if (phoneCriteria) {
            queryStrs.push(` LEFT JOIN \`Contact\` ON \`Patient\`.\`self_contact_id\` = \`Contact\`.\`id\`\n`);

            criteriaStrs.push(`\`Contact\`.\`mobile_phone\` LIKE ?  OR  `+
                              // `\`Contact\`.\`mobile_phone\` LIKE ?  OR `+
                              // `\`Contact\`.\`land_phone\` LIKE ?  OR  `+
                              `\`Contact\`.\`land_phone\` LIKE ?`  );

            criteriaList.push(phoneCriteria);
            // criteriaList.push('___'+phoneCriteria);
            criteriaList.push(phoneCriteria);
            // criteriaList.push('___'+phoneCriteria);
        }

        const emailCriteria = This._EmailSearchCriteria(searchStrs);
        if (emailCriteria) {
            if ( ! queryStrs.length ) {
                queryStrs.push(` LEFT JOIN \`Contact\` ON \`Patient\`.\`self_contact_id\` = \`Contact\`.\`id\`\n`);
            }
            criteriaStrs.push(`\`Contact\`.\`email\` LIKE ?  OR  \`Contact\`.\`alternate_email\` LIKE ?`);

            criteriaList.push(emailCriteria);
            criteriaList.push(emailCriteria);
        }
        ...
        This._ContemporaryDateSearchAdd(searchStrs, criteriaStrs, criteriaList, '`Patient`.`date_of_birth`');

        // logger.debug(`in PatientRecord DaoSearch() : \n`, queryStrs.join(''), criteriaStrs.join('  OR  ')+'\n', criteriaList);
        ...
        const results = await this.GetWithCriteria((       //  Add ._RecordSchema with all the right props to auto do the job.
            queryStrs.join('') +
            `WHERE \`Patient\`.\`feed_alias\` = ?  AND  \`Patient\`.\`row_retired\` IS NULL AND  (\n`+
            `      ${criteriaStrs.join(' OR\n      ')})\n`+
            `LIMIT 100`), criteriaList);
...
class PatientReachabilityJoined extends RecordJoined {
}
class PatientReachabilityRecord extends FeedRecord {
    #feed_item_id;
    static get TableName() { return 'Patient'; }
    static get uuidDbName() { return 'feed_item_id'; }
    static get Joined() { return new PatientReachabilityJoined(this); }

    constructor({feed_item_id, is_pehr_reachable, ...id__row_version__row_created__row_persisted__row_retired}) {

        super(id__row_version__row_created__row_persisted__row_retired);
        this.#feed_item_id = feed_item_id;
        this._setMostRecentKnownPersistedRowOwnUpdatableValues({ is_pehr_reachable, });
        // Object.assign(this, { is_pehr_reachable, });
    }

    //region The get/set "interface": {  feedItemId, isPehrReachable ... }

    get feed_item_id() { return this.#feed_item_id; }
    get feedItemId() { return this.#feed_item_id; }

    get isPehrReachable() { throw Error(`${this.Name}.prototype.get isPehrReachable() : Not defined yet. Run ${this.Name}.Setup().`); }
    // noinspection JSUnusedGlobalSymbols
    set isPehrReachable(value) { throw Error(`${this.Name}.prototype.set isPehrReachable(value=${value}) : Not defined yet. Run ${this.Name}.Setup().`); }
    ...
    toOwnLitOb() {
        const {  feedItemId, isPehrReachable, } = this;

        return { feedItemId, isPehrReachable, };
    }
    ...
    toNonRetiredApiLitOb(apiLitOb={}) {
        ...
        return {
            patientId: feedItemId,
            isReachable: isPehrReachable,
            ...apiLitOb
        };
    }
    ...
    static _OwnFields = {
        ...
        is_pehr_reachable(colName) {  return InsertAndUpdated({colName, recName:'isPehrReachable',  }); },
    };
    ...
    /**
     *
     * @param {{feedItemId: string, isPehrReachable: boolean,}} nativeSrcOb
     * @param {Error[]} validationErrors
     * @param {string[]} conflicts
     * @param {function} _dbInsert
     * @param {function} _dbUpdate
...
Object.assign(PatientReachabilityRecord, { DaoPushSingle, GetByUuid, }) ;
(self.PatientReachabilityRecord = PatientReachabilityRecord).Setup();
...
if ([false, true][ 0 ]) {
    ...
    // PatientRecord.GetWithCriteria('WHERE Patient.first_name = ?', [firstName]
        ...
        for (let record of records) {
            ...
            [   PatientRecord.Joined.birthPlace(),PatientRecord.Joined.motherBirthPlace(), PatientRecord.Joined.selfContact('selfContact'),
        ...
        if (records.length) {
            if ([false, true][ 0 ]) {                                           //  WARNING  :  DELETE ! ! !
                ...
                if ([false, true][ 0 ]) {
                     ...
                     */
                    const rec =  records[0];
                    const newName = ['Chaperon', 'Chaperonnette'][ 0 ];
                    rec.familyName = newName;
                    logger.info(`Changed patientRecord row : ${rec.toRowJSON()}`);

                    rec._update().then(({row_persisted, row_version}) => {
