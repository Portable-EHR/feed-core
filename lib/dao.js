...
class FeedRecord {
    ...
    static Setup() {
        ...
        thisProto._collectUpdateCandidateValues = function(candidateNativeSrcOb, validationErrors=[]) {
    ...
    async _update(conflicts=[], _dbUpdate=dbUpdate, _fetchFromDb=fetchFromDb) {
        ...
        //  DB triggers MUST be defined that take care of AUTO-INCREMENT-ing row_version.
        ...
        return this;   //  todo on transaction commit, move all #toUpdate properties values to the underlying object
    }
    ...
    async updateWithCandidate(nativeSrcOb, { validationErrors=[], conflicts=[], _dbInsert=dbInsert, _dbUpdate=dbUpdate,
        ...
        const { _uniJoinedRecordMapOb, constructor:This, idDbName, } = this;
        ...
        const updateWithJoinedSrcObs = async ({_dbInsert, _dbUpdate, _dbDelete, _fetchFromDb, trans}) => {
            ...
            for (let {UniOwnedFeedRecord, ownerPropertyName, recNameJoiningTo, colNameJoinedInOwned
                ...
                const   uniOwnedRecord =       this[ownerPropertyName],             //  e.g.    this.selfContact
                        uniOwnedSrcOb = nativeSrcOb[ownerPropertyName];             //   nativeSrcOb.selfContact
                ...
                if (uniOwnedRecord) {
                    ...
                    //  patientSrcOb[self_contact_id] =  contactRecord['id'];            //  calls the setter.
                    }
            ...
            for (let {UniOwnedFeedRecord, ownerPropertyName, recNameJoiningTo, colNameJoinedInOwner
                ...
                if (uniOwnedRecord) {
                    if (uniOwnedSrcOb) {    //  Both an instance fresh from Db and an update candidate are available
                    //  patientReachSrcOb[ patient_id ] = patientRecord['id'];
                        uniOwnedSrcOb[recNameJoiningTo] = this[colNameJoinedInOwner];
                ...
                //  patientReachSrcOb[ patient_id ] = patientRecord['id'];
                    ...
                    //  e.g.                ['reachability'] = patientReachabilityRecord;
                    _uniJoinedRecordMapOb[ownerPropertyName] = await UniOwnedFeedRecord.Insert({srcOb: uniOwnedSrcOb,
                                                        validationErrors, conflicts, _dbInsert, _fetchFromDb, trans});
                }
            }
                     ...
                     :  await doInTransaction((
                            async trans => {
                                await updateWithJoinedSrcObs({
                                            _dbInsert:trans.transDbInsert, _dbUpdate:trans.transDbUpdate,
...
class FeedItemRecord extends FeedRecord {
    ...
    static async DaoPullSingle(params, feed) {        //  'this' is the static This: the class, not the instance.
        ...
        try {
            ...
            return [feedItem.toApiLitOb(), feedItemId];
        }
    ...
    static async DaoRetireSingle(srcJsObFragment, feed) {
        ...
        try {
            ...
            return [This.prototype.toRetiredApiLitOb.call({feedItemId, feedAlias}, row_retired), feedItemId];
        }
    ...
    static async DaoUpdateSingle(srcLitOb, feed) {
        ...
        try {
            ...
            return [fresh.toApiLitOb(), feedItemId];
        }
        catch (e) {
