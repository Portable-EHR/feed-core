 ...
 *
 * @param {{prototype: FeedRecord}|function} FeedRecord
 * @param {{prototype: FeedRecord}|function} This
 ...
 * @constructor
 */
function Referenced({FeedRecord=FeedRecord, This, referenceIdProperty, referencePropertyName,
                        referenceIdLitObName, colNameJoiningToReferenced, colNameJoiningToReferencer, }) {

    const ThisRecord = FRproto(This).constructor;
    const ReferencedFeedRecord = FRproto(FeedRecord).constructor;

    const { name: referenceIdPropertyName } = referenceIdProperty;
 ...
 * @constructor
 */
function UniOwned({FeedRecord=FeedRecord, This, ownerProperty, ownerLitObName, colNameJoiningToOwned, colNameJoiningToOwner, }) {

    const ThisRecord = FRproto(This).constructor;
    const UniOwnedFeedRecord = FRproto(FeedRecord).constructor;

    const { name: ownerPropertyName } = ownerProperty;
 ...
 * @constructor
 */
function MultiOwned({FeedRecord=FeedRecord, This, ownerArrayProperty, ownerLitObArrayName, colNameJoiningToOwner, }) {

    const ThisRecord = FRproto(This).constructor;
    const MultiOwnedFeedRecord = FRproto(FeedRecord).constructor;

    const { name: ownerArrayPropertyName } = ownerArrayProperty;
...
class FeedRecord {
    ...
    static get Joined() { return new EmptyJoined(); }                                   //  Overridden in .Setup()
    ...
    static Setup() {
        ...
        let joinedProto = Object.getPrototypeOf(Joined);        //  get the [joinedProperty]() from all the prototypes
        while (joinedProto.constructor !== Object) {
            for (let propName of Object.getOwnPropertyNames(joinedProto)) {
                if (propName !== "constructor") {                                               //  Skip constructor.
                    const joinedPropertyFnc = joinedProto[propName];
                    if  ('function' === typeof joinedPropertyFnc) {
                        ...
                        Joined[propName] = () =>   params_Joined;           //  return the cached value, built once.
            ...
            joinedProto = Object.getPrototypeOf(joinedProto);
        }
