...
const { prototypesAlongTheProtoChain, collectNonOverriddenProtoFunctionsAlongTheProtoChain, zip, groupBy,
        toDate, dateAdd, strToDate, capitalizeName, niceJSON, decapitalizeName, normalizeString, normalizePhoneNumber,
 ...
 * @constructor
 */
function MultiOwned({joinName, FeedRecord=FeedRecord, ownerLitObArrayName, colNameJoiningToOwner, }) {
    //  HACK    ThisRecord was attached to new String(joinName) just before the call in FeedRecord.Setup().
    ...
    const params = BuildMultiOwnedFeedRecordParams({
                ...
                ownerLitObArrayName, colNameJoiningToOwner,
    });
...
class FeedRecord {
    ...
    static _BuildGetterSetterAndRecToCol({colName, _colName, recName, recEnum, canBeNull, sqlType, eType, typeParam}) {
        ...
        if (recEnum) {
            ...
            if ('date' === sqlType) {
                const errMsgSuffix = `] invalid : must be ${canBeNull ? 'null or ' : ''}a date in the range 1000-01-01 to 9999-12-31.`;
                ...
                //  To prevent this, a combination of regex and toDate() is first used to validate a datetime string,
                //  with the date portion in yyyy-mm-dd format and respecting 1000-01-01 to 9999-12-31 maximum range
                //  of MySql date. Then any potential 'T....Z' portion following the date portion (most likely none,
                //  or T00:00Z for an original 'yyyy-mm-dd' string) is discarded and replaced with 'T12:00Z'.
                ...
                if (canBeNull) {
                    validateNMorphToCol = recValue => {
                                            ...
                                            //  If recValue is already a jsDate, the pre-converter is in charge.
                                            recValue = ('string' !== typeof recValue  ||
                                                                          ! recValue.match(/^[1-9]\d{3}-\d{2}-\d{2}/) )
                                                        ?  recValue
                                                        :  toDate(recValue.slice(0, 10) + 'T12:00Z');
                                            if (recValue instanceof Date) {
                                                return recValue;
                                            }
                                            throw WrongType(errMsgPrefix + recValue + errMsgSuffix);
                                        };
                            //   colValue is a jsDate at 00:00 in local time, therefore in any time zone on earth :
                    morphToRec = colValue =>                //  add 12 h and date portion of UTC iso8601 will be right.
                        null === colValue  ?  undefined  :  dateAdd.hour(12, colValue).toISOString().slice(0, 10);
                }
                else {                                  //  cannotBeNull
                    validateNMorphToCol = recValue => {
                                            //  If recValue is already a jsDate, the pre-converter is in charge.
                                ...
                                //colValue is a jsDate at 00:00 in local time, therefore in any time zone on earth :
                    morphToRec =  colValue  =>  //  add 12 h and date portion of UTC iso8601 will be right.
                                                dateAdd.hour(12, colValue).toISOString().slice(0, 10);
                }
            }
            else {
        ...
        return searchStrs => {
            for (let str of searchStrs) {                                                       //  contemporary Date
                if (str.match(/^(18|19|20|21)\d{2}/) ) {
                    ...
                    const date = toDate((                       potentialDate.length === 4  ?
                                        potentialDate + '-01-01' :  potentialDate.match(/\d{4}-\d{2}$/)  ?
                                        potentialDate + '-01'    :
                                        potentialDate.slice(0,10)   ) + 'T12:00Z');

                    if (date instanceof Date) {                                     //  A valid at least start of a date
