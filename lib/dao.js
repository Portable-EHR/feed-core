...
const { collectNonOverriddenProtoFunctionsAlongTheProtoChain, zip, strToDate, capitalizeName, niceJSON,
...
const noThisRecordError = (joinedProperty, getter, extdClass) =>

                                            Error(`A class with a function property :\n\n\t${joinedProperty
                    }\n\nis instantiated by some FeedRecord .${getter} getter. That class MUST extends ${extdClass.name
...
function Referenced({FeedRecord=FeedRecord, referenceIdProperty, referencePropertyName,
    ...
    if ( ! ThisRecord) {
        throw noThisRecordError(referenceIdProperty, 'Joined', RecordJoined);
    }
...
function UniOwned({FeedRecord=FeedRecord, ownerProperty, ownerLitObName, colNameJoiningToOwned, colNameJoiningToOwner, }) {
    ...
    if ( ! ThisRecord) {
        throw noThisRecordError(ownerProperty, 'Joined', RecordJoined);
    }
...
function MultiOwned({FeedRecord=FeedRecord, ownerArrayProperty, ownerLitObArrayName, colNameJoiningToOwner, }) {
    ...
    if ( ! ThisRecord) {
        throw noThisRecordError(ownerArrayPropertyName, 'Joined', RecordJoined);
    }
