...
const { allProtoFunctionsAlongThePrototypeChain, zip, strToDate, capitalizeName, niceJSON,
        dbMsg, ErrorExtender, DeclareExpectedError, Enum, EItem, } = require('../../nodeCore/lib/utils');
...
const uniJoinedValidateAndComplete = (uniJoinedFeedRecordParams, { eJoinKind,
        ...
        if (colNameJoiningToDown) {         //  To{Owned|Referenced} OR To{Owner|Referencer}
            ...
            if ( ! colNameJoinedInDown ) {      // .colNameJoiningToDown is not a foreign key in This DB schema
                ...
                throw Error(
                    ...
                    }\` table \`${tableName}\` schema for ${name}, which includes single-column-foreign-key${
    ...
    const getFromRecordFieldToInsert = (Record, colName, unique='') => {
        ...
        if ( !fieldToInsert ) {
            ...
            throw Error(`${uniJoinedParams()} is based on DB \`${databaseName}\` table \`${Record.TableName
...
class RecordJoined {
    ...
    constructor(ThisRecord) {
        ...
        //  HACK
        //          Enforce the one-to-one relation between a FeedRecord and the RecordJoined class
        //          it instantiates in its .Joined static getter, attaching ThisRecord directly to
        //          functions of the prototypes along the chain, up to Object, singleton style.
        ...
        for (let [joinedPropertyFnc, joinedPropertyFncName, proto] of allProtoFunctionsAlongThePrototypeChain(this)) {
            if (proto !== thisProto) {      //  not this class own proto, but one that this class extends.

                //  steal a copy of it from extended class to avoid collision when attaching .ThisRecord
                //  (using bind to provide a distinct copy)

                if ( ! joinedPropertyFnc.name.startsWith('bound ') ) {   //  Not a drama if a function is bound more than once.
                    //  cache it in the instance directly,
                    //  all at once avoiding the bound fnc to be in the proto chain of an eventual extending class.
                    const bound = Object.defineProperty(joinedPropertyFnc.bind(this),'name',
                                                        {value: joinedPropertyFncName}); // avoid the "bound " prefix of bound fnc name.
                    this[joinedPropertyFncName] = Object.assign(bound, {ThisRecord});
                }
            //  else,  joinedPropertyFnc is either a bound or an arrow function. Real function needed to call bind.
            }
            else {
                const {ThisRecord: otherRecord} = joinedPropertyFnc;
                if (otherRecord) {                          //  Weird that it wasn't caught by isAssociatedToFeedRecord
                    throw doubleAssociationError(otherRecord);
                }
                Object.assign(joinedPropertyFnc, {ThisRecord,});    //  directly on the prototype function
            }                                                              //   singleton style
        }
    }
}
...
const noThisRecordError = joinedProperty =>
                                            Error(`A class with a function property :\n\n\t${joinedProperty
                    }\n\nis instantiated by some FeedRecord .Joined getter. That class MUST extends ${RecordJoined.name
                    }. Or that function, own a .ThisRecord property assigned with a FeedRecord class.\n`);
 ...
 * @constructor
 */
function Referenced({FeedRecord=FeedRecord, referenceIdProperty, referencePropertyName,
    ...
    if ('function' !== typeof referenceIdProperty) {
        throw Error(`${Referenced.name}({ referenceIdProperty }) .referenceIdProperty argument MUST be a `+
            `function() property of a class extending ${RecordJoined}.`);
    }

    const { name: referenceIdPropertyName, ThisRecord } = referenceIdProperty;

    if ( ! ThisRecord) {
        throw noThisRecordError(referenceIdProperty);
    }

    const params = BuildReferencedFeedRecordParams({
                        ThisRecord, ReferencedFeedRecord, referenceIdPropertyName, referencePropertyName,
                        referenceIdLitObName, colNameJoiningToReferenced, colNameJoiningToReferencer,
    });
 ...
 * @constructor
 */
function UniOwned({FeedRecord=FeedRecord, ownerProperty, ownerLitObName, colNameJoiningToOwned, colNameJoiningToOwner, }) {

    const UniOwnedFeedRecord = FRproto(FeedRecord).constructor;

    if ('function' !== typeof ownerProperty) {
        throw Error(`${UniOwned.name}({ ownerProperty }) .ownerProperty argument MUST be a `+
                    `function() property of a class extending ${RecordJoined}.`);
    }

    const { name: ownerPropertyName, ThisRecord } = ownerProperty;

    if ( ! ThisRecord) {
        throw noThisRecordError(ownerProperty);
    }

    const params = BuildUniOwnedFeedRecordParams({
        ThisRecord, UniOwnedFeedRecord, ownerPropertyName, ownerLitObName, colNameJoiningToOwned, colNameJoiningToOwner,
    });
 ...
 * @constructor
 */
function MultiOwned({FeedRecord=FeedRecord, ownerArrayProperty, ownerLitObArrayName, colNameJoiningToOwner, }) {

    const MultiOwnedFeedRecord = FRproto(FeedRecord).constructor;

    if ('function' !== typeof ownerArrayProperty) {
        throw Error(`${MultiOwned.name}({ ownerArrayProperty }) .ownerArrayProperty argument MUST be a `+
                    `function() property of a class extending ${RecordJoined}.`);
    }
    const { name: ownerArrayPropertyName, ThisRecord } = ownerArrayProperty;

    if ( ! ThisRecord) {
        throw noThisRecordError(ownerArrayPropertyName);
    }

    const params = BuildMultiOwnedFeedRecordParams({
                ThisRecord, MultiOwnedFeedRecord, ownerArrayPropertyName, ownerLitObArrayName, colNameJoiningToOwner,
    });
...
class FeedRecord {
    ...
    static get _MultiOwnedFeedRecordsParams() { throw Error(`${this.Name}.get _MultiOwnedFeedRecordsParams() : Not defined yet. Run ${this.Name}.Setup().`); }                                                                             //  read from DB schema by default, FeedRecord.idDbName by default
    ...
    static Setup() {
        ...
        for (let [joinedPropertyFnc, propFncName] of allProtoFunctionsAlongThePrototypeChain(Joined)) {
            //  calls one of UniOwned(), MultiOwned() and Referenced();  => { params, Joined }
            const params_Joined = joinedPropertyFnc.call(Joined, This); //  Build it once and cache it.
...
class FeedItemRecord extends FeedRecord {
    ...
    static async DaoRetireSingle(srcJsObFragment, feed) {
        ...
        try {
            const { feedItemId } = This.FromNonRetiredApiLitOb(srcJsObFragment);  // typically just get feedItemId
    ...
    static async DaoUpdateSingle(srcLitObDiff, feed) {
        ...
        try {
            ...
            const baseSrcItem = This.FromNonRetiredApiLitOb(srcLitObDiff);
