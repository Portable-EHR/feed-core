...
const { prototypesAlongTheProtoChain, collectNonOverriddenProtoFunctionsAlongTheProtoChain, zip,
        strToDate, capitalizeName, niceJSON, dbMsg, ErrorExtender, DeclareExpectedError,
...
class Field {
    constructor({colName, recName=colName, recEnum=null}, ThisRecord, _colNameFnc=colName_recName =>
        ...
        const {canBeNull, eType, typeParam,} = ThisRecord._ValidateFieldWithSchema(colName, recEnum);
        Object.assign(this, {
            Record: ThisRecord,
            ...
            ...ThisRecord._BuildGetterSetterAndRecToCol({colName, _colName, recName, recEnum,
                                                         canBeNull, eType, typeParam}), // => {recToCol(), set(), get()}
        });
    }
}
...
class FeedRecord {
    ...
    static _ValidateFieldWithSchema(colName, recEnum) {         //  'this' is the static This: the class constructor.
        ...
        if (undefined === schemaField) {
            throw Error(`${Name} colName [${colName}] not found in DB [${databaseName}] table [${TableName}].`)
        }
        ...
        if (eEnumDbJsType === eType) {
            if (null === recEnum) {
                throw EnumError(`No .recEnum specified for ${Name} .colName [${colName}] while DB [${
    ...
    static _BuildGetterSetterAndRecToCol({colName, _colName, recName, recEnum, canBeNull, eType, typeParam}) {
    ...
    static Setup() {
        ...
        for (let [joinedPropertyFnc, propFncName] of collectNonOverriddenProtoFunctionsAlongTheProtoChain(Joined)) {
            //  QUICK HACK
            //              Attach .ThisRecord to joinedPropertyFnc, just for the time of the call.
            joinedPropertyFnc.ThisRecord = This;

            //  calls one of UniOwned(), MultiOwned() and Referenced();  => { params, Joined }
            const params_Joined = joinedPropertyFnc.call(Joined);   //  Build it once and cache it.

            //  QUICK HACK
            //               Detach .ThisRecord from joinedPropertyFnc, once the call is done.
            delete joinedPropertyFnc.ThisRecord;
        ...
        const updating_ColNameMap = insertAndUpdatedFields.reduce(
