...
//region Feed and FeedHub ApiResponse  from nodecore lib/api

const { EFeedRequestStatus, BuildFeedApiResponse, EFeedHubRequestStatus,  } = require('../../nodecore/lib/api');
...
class FeedOp {
    ...
    handleApiError(e) {
        return e.isExpected ? { logMsg: this.logMessage(e) + '\n' +   expectedErrorLogMessage(e),
                                ownApiResponse:BuildFeedApiResponse({status:   expectedErrorStatus(e),
                                                                message:  expectedErrorShortMessage(e)})  }
                            : { logMsg: this.logMessage(e) + '\n' + unexpectedErrorLogMessage(e, this),
                                ownApiResponse:BuildFeedApiResponse({status: unexpectedErrorStatus( ),
