...
class PrivateMessageRecord extends FeedItemRecord {
    ...
    constructor({id, row_version, row_created, row_persisted, row_retired,  feed_alias, feed_item_id, backend_item_id,
                                                practitioner_id, patient_id, message_context, subject, text}) {
                                                    console.log('ssss',{feed_item_id,text,message_context})
     ...
     */
     ...
     static async DaoPullBundle({srcOb}) {
        ...
        PrivateMessage.*,
        PrivateMessageAttachment.doc_as_b64
        from PrivateMessage Join Patient on PrivateMessage.patient_id=Patient.id join Practitioner on PrivateMessage.practitioner_id=Practitioner.id left join  PrivateMessageAttachment on PrivateMessage.id=PrivateMessageAttachment.message_id order by PrivateMessage.id asc;`);
     
       return {results}
    }
    static async updateStatus(privateMessage, status) {
        return dbUpdate("UPDATE PrivateMessage set status=? where feed_item_id = UUID_TO_BIN(?,1)",
            [status.toString(), privateMessage.feedItemId]);
    }
    static async Insert({srcOb, validationErrors=[]}) {
        
        try {
            // console.log('feed',this.feed_item_id());
            // console.log('feed1',this.subject());
...
async function   fetchLatestMessage(){
   let _res= await fetchFromDb(`select PrivateMessage.id, BIN_TO_UUID(Patient.feed_item_id,1) as 'feed_item_id_to_send' , Practitioner.first_name as 'd_f_name', Practitioner.family_name as 'd_l_name' from PrivateMessage  Join Patient on PrivateMessage.patient_id=Patient.id  join  Practitioner on PrivateMessage.practitioner_id=Practitioner.id  order by PrivateMessage.id DESC limit 0,1;`);
   return _res[0];
}
self.fetchLatestMessage=fetchLatestMessage;
