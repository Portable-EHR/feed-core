...
const { feedHubEndpoint:{web}={}, feedProvider, Feed } = config,
...
class PostToFeedHub extends FeedOp {
    constructor(feed, path, command) {
        ...
        Object.defineProperty(this, '_command', {value:command});
    }
    ...
    static get FeedProviderFullTag() { return `FeedProvider [${feedProvider}] version []`; }
 ...
 */
const performFeedHubGet = async ({path, params, requiresJwt=true},
                                 {verbose=undefined, ...timeoutInMs_maxAttempts_extraOptions}={}) =>
        (await feedHubServerGet({path, params, requiresJwt}, {verbose, ...timeoutInMs_maxAttempts_extraOptions})).body;
self.performFeedHubGet = performFeedHubGet;



self.pingFeedHubServer = async (options={verbose:false}) =>
                                                await performFeedHubGet({path:'/node/ping', requiresJwt:false}, options);
