...
const { BackendApiResponse, } = require('../../nodeCore/lib/api');
const { eFlowsToBackend, Unpacking, BackendError, FeedOp } = require('../../nodeCore/lib/feed.core');
...
const performFeedHubPost = self.performFeedHubPost = async ({path, body:{command, parameters, trackingId = undefined, ...restOfBody
                                                        }={}}, {verbose=undefined, ...timeoutInMs_maxAttempts_extraOptions}={}) => {
    try {
        const {body, statusCode2XX:statusCode, msgHead} = await feedHubServerPost({path, postBody:{
                command, parameters, trackingId, ...restOfBody}}, {verbose, ...timeoutInMs_maxAttempts_extraOptions});
        let backendError;
        try {
            const jsOb = JSON.parse(body);
            // return BackendApiResponse(jsOb);
            const backendApiResponse = BackendApiResponse(jsOb);
            if (backendApiResponse.isStatusOk) {
                return backendApiResponse;
            }
            backendError = BackendError({msgHead, statusCode, ...backendApiResponse});
        }
        catch (e) {
            throw Unpacking(e, msgHead, `Error parsing JSON body in sendOnePostToBackend with backendRequest : { command : ${
