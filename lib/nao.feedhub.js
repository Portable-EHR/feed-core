...
const { feedHubEndpoint:{web:{Post:feedHubServerPost, Get: feedHubServerGet}={}}={}} = config;

const { FeedHubApiResponse, } = require('../../nodeCore/lib/api');
const { eFlowsToBackend, Unpacking, FeedHubError, FeedOp,
        expectedErrorTransportStatus, expectedFeedHubBackendErrorMaintenanceStatus } = require('./feed.core');

const self = module.exports;

Object.assign(self, {expectedErrorTransportStatus, expectedFeedHubBackendErrorMaintenanceStatus});     //  Own these.

class FeedHubOp extends FeedOp {get tag(){return `PostToFeedHub`;} get isToFeed(){return false;} get isOfFeed(){return false;}}
const no=false,
      feedOpFnc = endpoint =>
                        new (FeedHubOp.SetupFeedHub(FeedHubOp.prototype, eFlowsToBackend, no))(endpoint),
      feedOp = feedOpFnc(config.feedHubEndpoint);

self.feedOpFnc = feedOpFnc;

const _performFeedHubPost = self._performFeedHubPost = async (feedHubServerPost, feedOpFn,
                        {path, body:{feedAlias, command, parameters, trackingId = undefined, ...restOfBody}={}},
    ...
    try {
        ...
        throw Object.assign(e, {feedOp:feedOpFn()});
    }
};

const performFeedHubPost = self.performFeedHubPost = async ({path, body:{feedAlias, command, parameters, trackingId = undefined, ...restOfBody}={}}, {verbose=undefined, ...timeoutInMs_maxAttempts_extraOptions}={}) => {
    return await _performFeedHubPost(feedHubServerPost, ()=>feedOp,
        {path, body:{feedAlias, command, parameters, trackingId, ...restOfBody}},
