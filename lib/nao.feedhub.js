...
const { config } = require(process.env.PEHR_NODE_CWD+(process.env.PEHR_NODE_LIB_NODE || '/lib/node'));
 ...
 * @return {Promise<{requestStatus:({status:string, message:string}), responseContent:object}|FeedHubApiResponse>}
 */
    ...
    try {
        ...
        try {
            ...
            if (isFeedHubStatusOk(requestStatus)) {
                return ExtractFeedHubApiResponse({requestStatus, responseContent});
            }
