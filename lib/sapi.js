...
class Provider {
    ...
    bindFeedOps() {
        const { constructor:This, feed, feedTag } = this,
              { config:{feedProviderName}, verbose } = feed,
                fromTag = `from ${feedProviderName} ${feedTag}`,
                toTag = `to ${feedProviderName} ${feedTag}`,
                ofTag = `of ${feedProviderName} ${feedTag}`;
        /**
         *
         * @param {string} FeedItemName
         * @param {boolean|undefined} isApiSuccessVerbose
         * @param {string} fromToTags
         * @return {{isApiVerbose: boolean|undefined, FeedItemName: string, fromToTags: string}}
         */
        const feedOpValueProps = ({FeedItem:{FeedItemName, isApiSuccessVerbose,}}, fromToTags=ofTag) => ({
            FeedItemName,
            isApiSuccessVerbose,
            fromToTags
        });
        ...
        for (let [sapi_pullBundle, FeedItem] of This._PullBundles()) {
            ...
            this[name] = Object.assign(feedOp, feedOpValueProps({FeedItem}, fromTag));
        }
        ...
        for (let [sapi_pullSingle, FeedItem] of This._PullSingles()) {
            ...
            const feedOp =  {
                [name]: async function(params) {   //  this creates a named function
        ...
        for (let [sapi_pushSingle, FeedItem] of This._PushSingles()) {
            ...
            this[name] = Object.assign(feedOp, feedOpValueProps({FeedItem}, toTag));
        }

        for (let [sapi_addSingle, FeedItem] of This._AddSingles()) {

            const { name } = sapi_addSingle;
            const feedOp =  {[name]: function(srcLitOb) { 
        ...
        for (let [sapi_updateSingle, FeedItem] of This._UpdateSingles()) {
            ...
            this[name] = Object.assign(feedOp, feedOpValueProps({FeedItem}, ofTag));
        }
    ...
    practitionerFeedOps() {
        return FeedOps({
            sapiPullBundle: this.pullPractitionerBundle,
    ...
    patientFeedOps() { 
        return FeedOps({ 
            sapiPullBundle: this.pullPatientBundle,
            ...
            pullAll:this.pullAllPatient
        }); 
    }

    pullAllPatient(/*srcParams*/) { throw new Error("Provider.prototype.pullAllPatient : Override me !"); }
     ...
     * @returns {FeedOps}
     */
    privateMessageFeedOps() { 
        return FeedOps({ // sapiPullBundle: this.pullPatientBundle,
            sapiPullSingle: this.pullSinglePrivateMessageContent,
            sapiAddSingle:this.addSinglePrivateMessageContent,
            sapiPullBundle:this.pullPrivateMessageBundle
            // sapiUpdateSingle: this.updateSinglePatient,
            // sapiRetireSingle: this.retireSinglePatient,
            // sapiSearch: this.searchPatient,
        });
    }
    pullPrivateMessageBundle (){throw new Error("Provider.prototype.pullPrivateMessageBundle : Override me !");}
    pullSinglePrivateMessageContent(/*params*/) { throw new Error("Provider.prototype.pullSinglePrivateMessageContent : Override me !"); }
    addSinglePrivateMessageContent(/*params*/) { throw new Error("Provider.prototype.addSinglePrivateMessageContent : Override me !"); }
