...
class PatientRecord extends FeedItemRecord {
    ...
    static async DaoSearch(searchStr, feed) {
        const phoneNumber = normalizePhoneNumber(searchStr);
        const results = await this.GetWithCriteria((       //  Add ._RecordSchema with all the right props to auto do the job.
            ` LEFT JOIN \`Contact\` ON \`Patient\`.\`self_contact_id\` = \`Contact\`.\`id\`\n`+
            `WHERE \`Patient\`.\`feed_alias\` = ? AND  (\`Contact\`.\`mobile_phone\` = ?  OR  \`Contact\`.\`land_phone\` = ?)`+
            `LIMIT 100`), [
                                       feed.alias,                          phoneNumber,                        phoneNumber ]);
