...
const { FeedRecord, FeedItemRecord, ReferencedFeedRecordSetup, WrongType,
        Referenced, UniOwned, MultiOwned } = require('./dao');
...
class ContactRecord extends FeedRecord {
    ...
    static get Joined() { return new ContactJoined(); }
...
class FullContactJoined {
    address(This) {
        return UniOwned({FeedRecord:CivicAddressRecord, This, ownerProperty:this.address}); }
}
class FullContactRecord extends ContactRecord {

    static get Joined() { return new FullContactJoined(); }
...
//  static FromOwnedApiLitOb(ownedLitOb)    //  ContactRecord's already include ._AddJoinedFromApiLitOb()
    ...
    static get _FieldsToInsertAndUpdate() {
        ...
        return [
            ...
            //  .Joined UniOwned() entry, or as .colNameJoiningToReferenced of a
...
class MultiContactJoined extends FullContactJoined {
    addresses(This) {
        return MultiOwned({FeedRecord:MultiAddressRecord, This, ownerArrayProperty:this.addresses}); }
}
class MultiContactRecord extends ContactRecord {
    #addresses=[];

    static get Joined() { return new MultiContactJoined(); }
    //region The get/set "interface": { address }
...
class PatientJoined {
    primaryFeedPractitionerId(This) {
        return Referenced({FeedRecord:PractitionerRecord, This, referenceIdProperty:this.primaryFeedPractitionerId, referencePropertyName:'primaryPractitioner'}); }
    birthPlace(This) {
        return UniOwned({FeedRecord:BirthPlaceRecord,   This, ownerProperty:this.birthPlace,        ownerLitObName:'placeOfBirth',       colNameJoiningToOwned: 'birth_place_id'       }); }
    motherBirthPlace(This) {
        return UniOwned({FeedRecord:BirthPlaceRecord,   This, ownerProperty:this.motherBirthPlace,  ownerLitObName:'motherPlaceOfBirth', colNameJoiningToOwned: 'mother_birth_place_id'}); }
    /**
     *
     * @param  This
     * @returns {{params: JoinedParams, Joined: FullContactJoined}}
     */
    selfContact(This=PatientRecord) {
        return UniOwned({FeedRecord:FullContactRecord,  This, ownerProperty:this.selfContact,       ownerLitObName:'contact',               colNameJoiningToOwned: 'self_contact_id'      }); }
    emergencyContact(This) {
        return UniOwned({FeedRecord:ContactRecord,      This, ownerProperty:this.emergencyContact,                                          colNameJoiningToOwned: 'emergency_contact_id' }); }
    legitIds(This) {
        return MultiOwned({FeedRecord:PatientLegitIdRecord, This, ownerArrayProperty:this.legitIds, ownerLitObArrayName:'identifiedBy'}); }
}
class PatientRecord extends FeedItemRecord {
    #legitIds = [];
    static get TableName() { return 'Patient'; }
    static get Joined() { return new PatientJoined(); }
    ...
    toNonRetiredApiLitOb() {
        const { id, dateOfBirth, eGenderAtBirth, eGender, familyNameAtBirth, firstNameAtBirth,
...
if ([false, true][ 0 ]) {
    ...
    // PatientRecord.GetWithCriteria('WHERE Patient.first_name = ?', [firstName]
        ...
        for (let record of records) {
            ...
            [   PatientRecord.Joined.birthPlace(),PatientRecord.Joined.motherBirthPlace(),
