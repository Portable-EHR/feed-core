...
class PatientRecord extends FeedItemRecord {
    ...
    static async DaoSearch(searchStr, feed) {
        const criteriaList = [ feed.alias, ];
        const queryStrs = [];
        const criteriaStrs = [];

                            //  'allo Gi, 34 d35'  =>  ['allo', 'Gi', '34', 'd35']
        const searchStrs = searchStr.replace(/[\s,]+/g, ',').split(',');

        for (let [isPhoneElement, elementGen] of groupBy(searchStrs, subStr =>
                                                                                       ! subStr.match(/[^-().*0-9#]/))) {
            if (isPhoneElement) {
                const elements = Array.from(elementGen);
        ...
        for (let str of searchStrs) {
            ...
            if (mailMatch) {                                                                //  Email !
                ...
                const crit =  (pre ? ('%'+pre) : '') + '%@%' + (post ? (post+'%') : '');
                criteriaList.push(crit);
                criteriaList.push(crit);
                break;
            }
        }
        ...
        const potentialNames = searchStr.replace(/\s+/g, '' ).split(','),
              names = potentialNames.filter(potentialName =>
                                    ...
                                    .join('%');
        if (names) {
            criteriaStrs.push(`\`Patient\`.\`normalized_name\` LIKE ?`);
            criteriaList.push('%'+names+'%');
        }

        for (let str of searchStrs) {                                                           //  legit Id
            if ( str  &&  ! str.match(/[^-\w]/)) {
                queryStrs.push(` LEFT JOIN \`PatientLegitId\` AS \`legitId\` ON \`Patient\`.\`id\` = \`legitId\`.\`patient_id\`\n`);
        ...
        for (let str of searchStrs) {                                                       //  birthday
            if (str.match(/^(19|20)/) ) {
                let potentialDate = str.replace('/','-');
                if (potentialDate.endsWith('-')) {
                    potentialDate = potentialDate.slice(0, -1);
                }
                const date = onlyDate(potentialDate);
                if (date) {                                                     //  A valid at least start of a date
                    if (potentialDate.length > 7) {                                         //  A valid  yyyy mm dd
                        criteriaStrs.push(`\`Patient\`.\`date_of_birth\` = ?`);
                        criteriaList.push(date.toISOString().slice(0,10));
                    }
                    else {
                        criteriaStrs.push(`(\`Patient\`.\`date_of_birth\` >= ? AND \`Patient\`.\`date_of_birth\` < ? )`);
                        criteriaList.push(date.toISOString().slice(0,10));
                        if (potentialDate.length > 5) {                                     //  A valid  yyyy mm
                            criteriaList.push(dateAdd.month(1, date).toISOString().slice(0,10));
                        }
                        else {                                                              //  A valid  yyyy
                            criteriaList.push(dateAdd.year(1, date).toISOString().slice(0, 10));
                        }
                    }
                }
            }
