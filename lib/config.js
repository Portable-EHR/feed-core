...
class NodeConfig extends nodeCore_NodeConfig {
    ...
    constructor(srcJsOb, node) {
        ...
        const { netFlavorAllias, feedFlavorAlias, allCredentials, configFQN } = this;
        ...
        if (feedHubServerAlias  &&  undefined === feedHubServer) {  //  netFlavor-level feedHubServer is not mandatory for now.
            configLogger().bailOut(`netFlavor [${netFlavorAllias}] .feedHubServer: "${feedHubServerAlias}" definition not found in .feedHubServers {} section of config file [${configFQN}].`);
        }
            ...
            if ('string' === typeof feedHubCredentialsAlias ) {
                ...
                if(undefined === feedHubCredentials) {
                    configLogger().bailOut(`netFlavor [${netFlavorAllias}] .feedHubServer: "${
                                    feedHubServerAlias}" .credentials: "${feedHubCredentialsAlias
                ...
                configLogger().bailOut(`For netFlavor [${netFlavorAllias}] .feedHubServer [${feedHubServerAlias}] .scheme "${scheme
                }", the feedHubServer.credentials MUST specify an entry of the .credentials {} section of config file [${configFQN}].`);
            }
        }
        ...
        if ('string' === typeof feedFlavor) {
            ...
            configLogger().bailOut(`feedFlavor "${feedFlavorAlias
                            }" definition found in .feedFlavors {} section of config file [${configFQN
                            }]\n is : ${'object' === typeof feedFlavor  ?  niceJSON(feedFlavor)  :  `[${feedFlavor}]`
        ...
        for (let feedConfigAlias of feedFlavor) {
            ...
            if (undefined === feedConfig) {
                feedConfigErrMsgs.push(`feedConfig "${feedConfigAlias}" definition of feedFlavor "${feedFlavorAlias
                                        }" not found in .feedConfigs {} section of config file [${configFQN}].`);
            }
            ...
            if (undefined === previousFeed) {
                ...
                feedConfigErrMsgs.push(`${previousFeed.Name} with feedConfig [${previousFeed.feedConfigAlias}] and ${
                    feed.Name} with feedConfig [${feed.feedConfigAlias}] have the same .alias [${feed.alias
                }], but it MUST be unique across all feeds in .feedConfigs {} section of config file [${configFQN}].`);
            }
        }

        if (_invalidFeedHubServerMsgs.length)   feedConfigErrMsgs.push(`In config file [${configFQN}] :\n${_invalidFeedHubServerMsgs.join('\n')}`);
    ...
    _newFeedHubEndpoint(feedHubServer, credentials) {
        ...
        const endpoint = new Endpoint(feedHubServer, this,
