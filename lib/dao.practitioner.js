...
class PractitionerRecord extends FeedItemRecord {
    ...
    static async DaoPullBundle(srcPullParams, feed) {
        const o = await super.DaoPullBundle(srcPullParams, feed);   //  o is bundleResponse: { offset, hasMore, results}

        o.results = o.results.map(({ feedAlias, feedItemId, firstName,lastName, middleName, lastUpdated, practices }) =>
            ({ feedAlias, feedItemId, firstName, lastName, middleName, lastUpdated, practices:
                    practices.map(({ issuer, number, isActive }) => ({ issuer, number, isActive })), }));
        logger.debug(o);
