...
const { dateAdd, normalizeString, dbMsg, } = require('../../nodeCore/lib/utils');
...
class PractitionerLegitIdRecord extends FeedRecord {
    ...
    static FromOwnedApiLitOb(ownedApiLitOb, nativeLitOb) {
        ...
        return {           //  todo change this Backend format ? to match patient's and at least to carry expiresOn
            ...
            backendItemId,
            eIssuerKind,
            issuerAlias: issuerAlias ? issuerAlias : issuer,
            number,
            version,
    ...
    static async DaoSearch(searchStr, feed) {
        ...
        const potentialNames = searchStr.replace(/\s+/g, '' ).split(','),
            names = potentialNames.filter(potentialName =>
                                  ...
                                  .join('%');
        if (names) {
            criteriaStrs.push(`\`Practitioner\`.\`normalized_name\` LIKE ?`);
        ...
        for (let str of searchStrs) {                                                           //  legit Id
            if ( str  &&  ! str.match(/[^-\w]/)) {
                queryStrs.push(` LEFT JOIN \`PractitionerLegitId\` AS \`legitId\` ON \`Practitioner\`.\`id\` = \`legitId\`.\`practitioner_id\`\n`);
