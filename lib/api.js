...
const { BuildFeedApiResponse, EFeedRequestStatus } = require('../../nodecore/lib/api');
const {
...
const handleApiSuccess = (res, message, responseContent={}) => reply(res, BuildFeedApiResponse({message, responseContent}));
self.handleApiSuccess = handleApiSuccess;

const handleApi = (res, status, message) => reply(res, BuildFeedApiResponse({status, message}));
...
const unexpectedError = (e, msg) => ({
    ...
    ownApiResponse:BuildFeedApiResponse({status:eFeedRequestStatusInternal, message:`${msg}${e.isExpected ? `  ${e.message}` : ''}`}),
});
...
const applyHandlerForCommand = (handlerForCommand, isRoleAllowedForCommand=(/*command*/)=>
    ...
    async (req, res) => {
        ...
        try {
            ...
            if ( ! isRoleAllowed ) {
                return defaultApiErrorHandling(req, res, AuthError(BuildFeedApiResponse({
...
const pullBundle = sapiPullBundle =>
    async (res, sapi, params) => {
        ...
        try {
             ...
             */
            const bundle = await feedOp(params);                                                     //  May throw!
