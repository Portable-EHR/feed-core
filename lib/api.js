...
const pullSingle = sapiPullSingle =>
    async (res, sapi, params) => {
        ...
        try {
            const [feedItem, feedItemId] = await feedOp(params);                                                      //  May throw!
            const message = `Returning ${feedOp.FeedItem.FeedItemName} [${feedItemId}] from ${sapi.feedTag}.`;
...
const updateSingle = sapiUpdateSingle =>
    async (res, sapi, params) => {
        ...
        try {
            const [feedItem, feedItemId] = await feedOp(params);                                                      //  May throw!
            const message = `Updated ${feedOp.FeedItem.FeedItemName} [${feedItemId}] of ${sapi.feedTag}.`;
            if (sapi.verbose) logger.trace(`${message}\n${JSON.stringify(feedItem)}`);
            handleApiSuccess(res, message, feedItem);
        }
        catch (e) {
...
const retireSingle = sapiRetireSingle =>
    async (res, sapi, params) => {
        ...
        try {
            const [feedItem, feedItemId] = await feedOp(params);                                                      //  May throw!
            const message = `Retired ${feedOp.FeedItem.FeedItemName} [${feedItemId}] from ${sapi.feedTag}.`;
            if (sapi.verbose) logger.trace(`${message}\n${JSON.stringify(feedItem)}`);
            handleApiSuccess(res, message, feedItem);
        }
        catch (e) {
...
const pushSingle = sapiPushSingle =>
    async (res, sapi, params) => {
        ...
        try {
            const [feedItem, feedItemId] = await feedOp(params);                                                      //  May throw!
            const message = `Pushed ${feedOp.FeedItem.FeedItemName} [${feedItemId}] to [${sapi.feedTag}].`;
...
const search = sapiSearch =>
    async (res, sapi, params) => {
        ...
        try {
            ...
            const message = `Returning an array of [${searchResults.length}] ${feedOp.FeedItem.FeedItemName} search results from ${sapi.feedTag}.`;
