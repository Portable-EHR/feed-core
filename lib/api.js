 ...
 */
const applyHandlerForCommand = (handlerForCommand) => async (req, res) => {
    self.logRequest(req, logger);
    ...
    try {
        ...
        const feed = await authorizeApiRequest(req.headers, {feedAlias, Feed}, req.pEhr_isRoleAllowed);
...
const handleReq = (feedOps) => {
    ...
    const {sapiPullBundle, sapiPullSingle, sapiPushSingle, sapiAddSingle, sapiUpdateSingle, sapiRetireSingle} = feedOps;
    ...
    //  }[command])
    return applyHandlerForCommand(command => commandsAndHandlers[command]);
