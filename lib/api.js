...
const {PushSinglePrivateMessageNotification} = require("./feedhub.ops");
...
const { config: {Feed} } = require(process.env.PEHR_NODE_CWD+(process.env.PEHR_NODE_LIB_NODE || '/lib/node'));
const { niceJSON, html, dbMsg, } = require('../../nodecore/lib/utils');
...
const {fetchLatestMessage,PrivateMessageRecord,EPrivateMessageStatus} =require('./my-dao.privateMessage')
...
self.handleGetError = (req, res, e) => {
    if (e instanceof AuthError) {
        ...
        logger.error(`${headMsg} on ${req.method.toLowerCase()}(${req.originalUrl}) request: ${message}`);
        reply(res, html.pre(message));
    }
    else {
        logger.error(`On ${req.method.toLowerCase()}(${req.originalUrl}) request :\n` + dbMsg(e));
...
const handleUnknownCommand = self.handleUnknownCommand = (req, res, command) => {
    const message = `${req.method.toLowerCase()}(${req.originalUrl}) command [${command}] unknown.`;
...
const defaultApiErrorHandling = self.defaultApiErrorHandling = (req, res, e) => {
    if (e instanceof AuthError) {
        ...
        logger.error(`${message} on ${req.method.toLowerCase()}(${req.originalUrl}) request: ${ownApiResponse.message}`);
        return reply(res, ownApiResponse);
    }
    const message =  `Error on ${req.method.toLowerCase()}(${req.originalUrl}) request : `;
    logger.error(message, e);
    self.handleMalformed(res, message + e.message);
};
...
const applyHandlerForCommand = (handlerForCommand, isRoleAllowedForCommand=(/*command*/)=>
    ...
    async (req, res) => {
        ...
        try {
            ...
            const isRoleAllowed = isRoleAllowedForCommand(command);
            if ( ! isRoleAllowed ) {
                return defaultApiErrorHandling(req, res, AuthError(FeedApiResponse({
                        status:eFeedRequestStatusInternal,
                        message: `No isRoleAllowed() function provided for command [${command}]`
                    })));
            }

            // May throw AuthError on auth and feed-not-found failure
            const feed = await authorizeApiRequest(req.headers, {feedAlias, Feed}, isRoleAllowed);
...
const pullBundle = sapiPullBundle =>
    async (res, sapi, params) => {
        ...
        try {
            /**
             * @type {{results:object[], offset: number, hasMore:boolean}}
             */
            const bundle = await feedOp(params);   
                                                             //  May throw!
            const message = `Returning a Bundle of [${bundle.results.length}] ${feedOp.FeedItemName} ${feedOp.fromToTags}.`;
            if (sapi.verbose || feedOp.isApiSuccessVerbose) logger.trace(`${message}\n${JSON.stringify(bundle)}`);
            handleApiSuccess(res, message, bundle);
        }
        catch (e) {
...
const pullSingle = sapiPullSingle =>
    async (res, sapi, params) => {
        ...
        try {
            ...
            const message = `Returning ${feedOp.FeedItemName} [${feedItem._feedItemId}] ${feedOp.fromToTags}.`;
...
const addSingle = sapiAddSingle =>
    async (res, sapi, params) => {
        ...
        try {
            const feedItem = await feedOp(params);
            if(feedItem.messageId!==undefined){
                const latestMessage=await fetchLatestMessage();
                const uuid = uuidv4();
              
               
                //sending Notification to Feedhub
                const pushSinglePrivateMessageNotification = new PushSinglePrivateMessageNotification(sapi.feed,
                    {feedItem: {
                        messageId: feedItem.feedItemId,
                        patientId: latestMessage.feed_item_id_to_send,
                        dateCreated: feedItem.dateCreated,
                        author: latestMessage.d_f_name+' '+latestMessage.d_l_name,
                     }, trackingId:uuid},
                    {verbose:sapi.feed.verbose});
                    const pushPMResponse = await pushSinglePrivateMessageNotification.pushToDst()
                    console.log({pushPMResponse})
    
            }                                             //  May throw!
            const message = `Added ${feedOp.FeedItemName} ${feedOp.fromToTags}.`;
            if (sapi.verbose || feedOp.isApiSuccessVerbose) logger.trace(`${message}\n${JSON.stringify(feedItem)}`);

            if('OK' == pushPMResponse.requestStatus.status){
                await PrivateMessageRecord.updateStatus(feedItem, EPrivateMessageStatus.sent);
                handleApiSuccess(res, message, feedItem);
            } else {
                await PrivateMessageRecord.updateStatus(privateMessage, EPrivateMessageStatus.failed);
                handleApi(res, pushPMResponse.requestStatus, pushPMResponse.message);
            }
...
const updateSingle = sapiUpdateSingle =>
    async (res, sapi, params) => {
        const feedOp = sapi[sapiUpdateSingle.name];
        try {
            const feedItem = await feedOp(params);                                                      //  May throw!
            const message = `Updated ${feedOp.FeedItemName} [${feedItem._feedItemId}] ${feedOp.fromToTags}.`;
            if (sapi.verbose || feedOp.isApiSuccessVerbose) logger.trace(`${message}\n${JSON.stringify(feedItem)}`);
            handleApiSuccess(res, message, feedItem);
        }
        catch (e) {
...
const retireSingle = sapiRetireSingle =>
    async (res, sapi, params) => {
        ...
        try {
            const feedItem = await feedOp(params);                                                      //  May throw!
            const message = `Retired ${feedOp.FeedItemName} [${feedItem._feedItemId}] ${feedOp.fromToTags}.`;
            if (sapi.verbose || feedOp.isApiSuccessVerbose) logger.trace(`${message}\n${JSON.stringify(feedItem)}`);
            handleApiSuccess(res, message, feedItem);
        }
        catch (e) {
...
const pushSingle = sapiPushSingle =>
    async (res, sapi, params) => {
        const feedOp = sapi[sapiPushSingle.name];
        try {
            const feedItem = await feedOp(params);                                                      //  May throw!
            const message = `Pushed ${feedOp.FeedItemName} [${feedItem._feedItemId}] ${feedOp.fromToTags}.`;
            if (sapi.verbose || feedOp.isApiSuccessVerbose) logger.trace(`${message} ${JSON.stringify(feedItem)}`);
            handleApiSuccess(res, message, feedItem);
        }
        catch (e) {
...
const search = sapiSearch =>
    async (res, sapi, params) => {
        ...
        try {
            const { seq, searchStr } = params;
            const searchResults = await feedOp(searchStr);                                              //  May throw!
            const message = `Returning an array of [${searchResults.length}] ${feedOp.FeedItemName} search results ${feedOp.fromToTags}.`;
            if (sapi.verbose || feedOp.isApiSuccessVerbose) logger.trace(`${message}\n${JSON.stringify(searchResults)}`);
            handleApiSuccess(res, message, { seq, searchResults });
        }
        catch (e) {
