...
const { FeedRecord, FeedItemRecord, RecordJoined, MultiOwned, OnlyInserted, InsertAndUpdated } = require('./my-dao');
...
class PractitionerLegitIdRecord extends FeedRecord {
    ...
    //region The get/set "interface": {  practitioner_id, feedItemId, backendItemId, eIssuerKind, issuerAlias, number, version, expiresOn, ... }
...
class PractitionerRecord extends FeedItemRecord {
    ...
    //region The get/set "interface": {  family_name, first_name, middleName, normalizedName, legitIds }
        ...
        if (records.length) {
            if ([false, true][ 0 ]) {                                           //  WARNING  :  DELETE ! ! !
                ...
                if ([false, true][ 0 ]) {
                     ...
                     */
                    const rec =  records[0];
                    const newName = ['Bessette', 'Bingo'][ 0 ];
