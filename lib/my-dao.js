...
class FeedRecord {
    ...
    static FromNonRetiredApiLitOb(apiLitOb, nativeLitOb={}) {
        ...
        return this._AddJoinedFromApiLitOb(apiLitOb, {
            ...
            subject:apiLitOb?.subject,
            messageContext:apiLitOb?.messageContext,
    ...
    static BuildFromRow(row) {   
        //return row;                               //  'this' is the static This: the class constructor.
        ...
        //          and fill recordMaps from the Records who are owners of multiOwnedRecords.
         ...
         for (let [Record, {sqlSelectFrom, OwnerTblName, colNameJoiningTo, propertyName}] of This._MultiSqlSelectFroms) {
            ...
            if (undefined === colNameJoiningTo) {           //  The top Owner: not a multiOwnedFeedRecord :
                ...
                multiOwnedRecords.push(Object.assign(fetchedRecords, { OwnerTblName, colNameJoiningTo, propertyName}));
            }
        }
...
class FeedItemRecord extends FeedRecord {
    ...
    static async GetByUuid(uuid, feedAlias=null, _fetchFromDb=fetchFromDb) {
        ...
        const [sqlFeedAlias, args] =  null === feedAlias  ?  ['', [uuid]] :  [' AND `'+TableName+'`.`feed_alias` = ?',[uuid,feedAlias]];

        const criteriaString = 'WHERE `'+TableName+'`.`'+uuidDbName+'`'+_AssignStrByColName(uuidDbName) + sqlFeedAlias; //  Don't exclude row_retired IS NOT NULL: toApiLitOb() has a .retired version
        let data = await This.GetWithCriteria(criteriaString, args, _fetchFromDb);
        console.log("data ==>", data);
        return await This._FetchById(uuid, uuidDbName, This.GetWithCriteria(criteriaString, args, _fetchFromDb), feedAlias)
    }
