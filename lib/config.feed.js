...
const EFeedKind = (f=>{f.prototype=new Enum(f); return new f({});})(function EFeedKind({
    ...
    dispensaryProxy =(f=>f(f))(function dispensaryProxy(f)  { return EItem(EFeedKind, f); }),
    teleClinic      =(f=>f(f))(function teleClinic(f)       { return EItem(EFeedKind, f); }),
}) {  Enum.call(Object.assign(this, {dispensary, dispensaryProxy, teleClinic, })); });
self.EFeedKind = EFeedKind;

const {
    dispensary:         eDispensaryKind,
    dispensaryProxy:    eDispensaryProxyKind,
    teleClinic:         eTeleClinic,
} = EFeedKind;
[eDispensaryKind, eDispensaryProxyKind, eTeleClinic, ].join();    //  Kludge to prevent stupid 'unused' warnings.
...
class Feed {
    ...
    async initialize(subAction=async ()=>{}) {
        ...
        Object.defineProperty(this, '_sapi', {value: new (require(process.env.PEHR_NODE_CWD+(process.env.PEHR_NODE_LIB_SAPI || '/lib/sapi')).Provider)(this)});
